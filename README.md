...
```bash
npm install react-accessibility styled-components
```
```bash
yarn add react-accessibility styled-components
```
...
```tsx
import React from 'react';
import { AccessibilityWidget, WidgetTheme } from 'react-accessibility';
